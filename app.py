...
def get_recent_albums_fallback(limit=25):
    """
    Fallback method to get recent albums by scanning the entire MPD database.
    Sorts all albums by their newest file modification time.
    """
    ...
    try:
        print("Scanning entire MPD database for recent albums...")
        all_files = client.listallinfo()
        ...
        for song in all_files:
            ...
            artist_name = artist_name.strip() if artist_name else 'Unknown Artist'
            
            if not album_name:
                continue
                
            album_key = f"{artist_name} - {album_name}"
            albums_dict[album_key].append(song)
        
        # Convert to album info with newest file date
        album_info_list = []
        
        for album_key, songs in albums_dict.items():
            # Find the newest file in this album
            newest_time = 0
            sample_file = songs[0]['file'] if songs else None
            
            for song in songs:
                if 'last-modified' in song:
                    try:
                        # Parse MPD's ISO format timestamp
                        from datetime import datetime
                        dt = datetime.fromisoformat(song['last-modified'].replace('Z', '+00:00'))
                        song_time = dt.timestamp()
                        if song_time > newest_time:
                            newest_time = song_time
                            sample_file = song['file']
                    except:
                        pass
            
            # Skip albums without valid timestamps
            if newest_time == 0:
                continue
            ...
            # Extract artist and album from album_key
            artist_name = album_key.split(' - ')[0]
            album_name = ' - '.join(album_key.split(' - ')[1:])
        ...
        album_info_list.sort(key=lambda x: x['date'], reverse=True)
        
        client.disconnect()
        elapsed = time.time() - start_time
        print(f"Fallback scan complete: {len(album_info_list)} albums found in {elapsed:.2f} seconds")
        
        return album_info_list[:limit]
        
    except Exception as e:
        print(f"Error in fallback scan: {e}")
        if client:
            client.disconnect()
        return []
...
def get_recent_albums_from_mpd(limit=25, force_refresh=False):
    """
    Get the most recently modified albums from MPD with intelligent caching.
    Can be configured via RECENT_MUSIC_DIRS environment variable for specific directories,
    or falls back to scanning the entire music library efficiently.
    """
    import time
    global recent_albums_cache, recent_albums_cache_mod_times
    
    # Use environment-based configuration with fallback to full library scan
    recent_dirs_env = os.environ.get('RECENT_MUSIC_DIRS', '')
    if not recent_dirs_env.strip():
        return get_recent_albums_fallback(limit)
    else:
        # For now, fall back to full scan even if specific directories are configured
        # TODO: Implement specific directory scanning for better performance
        return get_recent_albums_fallback(limit)
    
    # Simple change detection cache - safe fallback to normal scan
    try:
        # Get recent music directories from environment variable
        recent_dirs_env = os.environ.get('RECENT_MUSIC_DIRS', '')
        if recent_dirs_env.strip():
            # Split by comma and clean whitespace
            directories_to_check = [d.strip() for d in recent_dirs_env.split(',') if d.strip()]
            print(f"Getting recent albums from '{recent_dirs_env}' directories...")
        else:
            # Fallback: scan entire music library if no specific directories configured
            directories_to_check = ['']  # Empty string means scan all
            print("No specific recent directories configured, scanning entire music library...")
        
        # Keep the old hardcoded values as a last resort fallback
...
def api_album_tracks():
    ...
    try:
        # Search for tracks by album and optionally by artist for more precise results
        if artist:
            # Search by both album and artist for more accurate results
            tracks = client.find('album', album, 'artist', artist)
            print(f"[DEBUG] Searching with album='{album}' AND artist='{artist}'")
        else:
            # Fall back to album-only search
            tracks = client.find('album', album)
            print(f"[DEBUG] Searching with album='{album}' only")
        
        print(f"[DEBUG] MPD returned {len(tracks) if tracks else 0} tracks", flush=True)
        
        # If no tracks found with artist filter, try without artist filter
        if not tracks and artist:
            print(f"[DEBUG] No tracks found with artist filter, trying album-only search")
            tracks = client.find('album', album)
            print(f"[DEBUG] Album-only search returned {len(tracks) if tracks else 0} tracks", flush=True)
